        ...
        <div class="row">
			...
			<input type="hidden" name="id" value="<?php echo ($pd["id"]); ?>">
			  <div class="layui-form-item">
				<label class="layui-form-label">网银通道名称：</label>
			  ...
			  <div class="layui-form-item">
				<label class="layui-form-label">网银通道代码（英文字符）：</label>
				<div class="layui-input-block">
			  ...
			  <div class="layui-form-item">
				...
				<select name="pd[paytype]" lay-verify="required" id="paytypes" lay-search="">
				  <option value="">直接选择或搜索选择</option>
					<?php if(is_array($paytypes)): $i = 0; $__LIST__ = $paytypes;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$b): $mod = ($i % 2 );++$i;?><option <?php if($pd[paytype] == $b[id]): ?>selected<?php endif; ?> value="<?php echo ($b["id"]); ?>"><?php echo ($b["name"]); ?></option>
					<!--<option <?php if($pd[paytype] == 5): ?>selected<?php endif; ?> value="5">网银跳转</option>-->
					<!--<option <?php if($pd[paytype] == 6): ?>selected<?php endif; ?> value="6">网银直连</option>--><?php endforeach; endif; else: echo "" ;endif; ?>
				  </select>
				</div>
			  </div>
...
layui.use(['layer', 'form'], function(){
  ...
  form.on('submit(add)', function(data){
    $.ajax({
		url:"<?php echo U('Channel/saveBank');?>",
		type:"post",
