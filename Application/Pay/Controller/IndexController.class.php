...
class IndexController extends PayController
{
    ...
    protected function productIsOpen()
    {
        ...
        $channel = M('Channel')->where(['id' => $this->channel['channel']])->find();
        if ($channel['paytype'] == 5) {
            //网银支付 加入bankid计算
            $this->bankid = I('request.pay_bankid', '');
            if ($this->bankid == '') {
                $this->showmessage('银行编码有误');
            }
            $bank_channel = M('Banks')->where(['status' =>1, 'code' => $this->bankid])->find();
            if (!$bank_channel) {
            ...
            $this->channel = $bank_channel;
            $this->channel['userid'] = $userid;
            $this->channel['pid'] = $pid;
        }
    }
