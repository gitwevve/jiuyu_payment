...
class WithdrawalController extends BaseController
{
    ...
    public function payment()
    {
        ...
        if($extends) {
            $where['extends'] = array('eq', $extends);
        }
    ...
    public function exportweituo()
    {
        ...
        $title = array('类型', '商户编号', '通道名称', '通道商户号', '系统订单号', '商户订单号', '处理号', '结算金额', '手续费', '到账金额', '银行名称', '支行名称', '银行卡号', '开户名', '所属省', '所属市', '申请时间', '处理时间', '状态', "备注");
    ...
    public function editStatus()
    {
        ...
        if (IS_POST) {
            ...
            $requestData = ['id' => $id];
            if (!$verified) {
                $this->redirect('Auth/verifyGoogle', ['redirect' => 'Withdrawal-editwtStatus', 'requestData' => base64_encode(json_encode($requestData))]);
            } else {
    ...
    public function saveBlacklist()
    {
        if (IS_POST) {
            ...
            //保存
            if ($id) {
                $res = M('BankBlacklist')->where(['id' => $id])->save($rows);
            } else {
                $res = M('BankBlacklist')->add($rows);
            }
            $this->ajaxReturn(['status' => $res]);
        }
    }

    public function editBlacklistStatus()
    {
        if (IS_POST) {
            $id    = intval($_POST['id']);
            $colum = I('post.k');
            $value = I('post.v');
            $res   = M('BankBlacklist')->where(['id' => $id])->save([$colum => $value]);
            $this->ajaxReturn(['status' => $res]);
        }
    }
}
